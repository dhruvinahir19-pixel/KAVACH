"""
features_core.py — THE feature builder (single source of truth).

Pure function: six EOD frames in (engine store schemas) -> feature panel out.
Shared byte-identically by the research engine (features.py) and the live scanner
(scanner/kcore/eod_cache.py + evening.py). Parity between backtest and live signals
is enforced by tests/parity against this module. DO NOT fork this logic.
...
Labels use t+1 data only.
"""
import numpy as np
import pandas as pd


def build_features(cash, fut, opt, part, mkt, vix):
    """cash/fut/opt/part/mkt/vix: DataFrames with the engine store schemas.
    Returns the full feature panel (all dates present in the inputs)."""
    ...
    # ---------------------------------------------------- market layer
    # NOTE (P2 audit finding, inherited from research code — DO NOT "fix"
    # without retraining + revalidating): after this sort the panel is in
    # (date, symbol) order, so the row-over-row diffs below are ~0.0 for all
    # but one symbol per day. vix_chg1 / vix_chg5 / nifty_ret1 are therefore
    # effectively-dead features (~constant 0). They were dead during research
    # training AND validation AND are dead in production scoring, so the
    # validated metrics (52.7% / AUC 0.709 / capture 1.48x) already price this
    # in. No look-ahead, no leakage — just no signal. vix LEVEL is correct.
